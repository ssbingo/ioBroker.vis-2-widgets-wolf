...
适配器附带 ioBroker 脚本 `gasverbrauch_statistik_v3.0.0.js`（目录 `addOn/`）。它根据表读数计算今天、昨天、最近 7 天和 30 天以及本月和上月的用量，并保存为对象（默认在 `0_userdata.0.Gas`）——正是 CCU 网页界面为 HmIP-ESI 显示、却无法通过 `hm-rpc` 进入 ioBroker 的那些数值。日常运行不需要历史适配器。 自 3.0 版起，它还能校准抄表读数、计算费用（含基本费、增值税与预付款），并通过 Telegram 和电子邮件发送日报与月度账单。

在 `javascript` 适配器中新建脚本，把 `SRC` 指向表读数并启动。随后在燃气表小部件的 *统计脚本* 中选择该目录——状态会自动填入。磁贴显示哪些数值，由 *显示的数值* 分组决定。

安装后脚本也可在浏览器中获取：`http://<iobroker>:8082/vis-2/widgets/vis-2-widgets-wolf/addon/gasverbrauch_statistik_v3.0.0.js`。详情见[英文文档](../../README.md)。
...
### 0.8.1 (2026-09-25)
* (ssbingo) 附带统计脚本：当夜间日报与日结落在同一分钟时会被悄悄丢弃——防止重叠运行的锁会丢掉第二次调用，而不是排队。现在日结在 00:00 运行，日报仍在 00:01，并用串行队列追加任务而非丢弃，这也能挽救在预填充期间到达的表读数变化
* (ssbingo) addOn/ 目录中只保留当前脚本版本；此前简版手册仍指向已删除的版本

### 0.8.0 (2026-09-23)
* (ssbingo) 燃气表：费用计算新增*增值税 (%)* 字段。此前磁贴只把单价与基本费相加，结果是含税还是不含税取决于填写的价格；而附带脚本按净价计算并加上税额，两者相差 19%。默认值 0 不会改变现有配置

### 0.7.1 (2026-09-23)
* (ssbingo) 燃气表：选择脚本目录时也会填入已校准的表读数与计算得到的流量——此前若这两个字段已指向传感器则保持不变，磁贴因此仍显示未校准的读数
* (ssbingo) 燃气表：若读数来自脚本却仍设置了修正值，磁贴会给出提示，否则会重复修正

### 0.7.0 (2026-09-23)
* (ssbingo) 燃气表：新增对象字段“本月费用”——关联后磁贴显示该数值，而不再按费率自行计算；这与附带脚本相配，其金额已含基本费、增值税与预付款
* (ssbingo) 附带统计脚本升级到 3.0.0：可校准抄表读数（针对丢失脉冲的修正系数）、计算费用，并通过 Telegram 与电子邮件发送日报和月报，可附 PDF
* (ssbingo) 选择脚本目录时也会填入费用对象；手册列出了新的设置项与状态
